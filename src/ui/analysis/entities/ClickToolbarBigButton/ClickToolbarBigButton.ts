...
//нажатие по кнопкам toolbar bigButton
export class ClickToolbarBigButton extends CodeEntity {
    getPrevCode(): string {
    ...
    getCode(): string {
        ...
        return (
            this.getPrevCode() +
            `ToolbarUtils.clickToolbarButton(${container},"${
                this.prevRecord?.bigButton
            }","${this.record.bigButton?.replace(" v", "")}");`
        );
    }
}
